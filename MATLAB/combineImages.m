...
global imageDatasets rootDir MARGIN newImage outImage

root = 'B:\Users\eric_000\Documents\Programming\Chris\';
datasetName = '3mo wmSVZ-2';
readMetaData(fullfile(root, datasetName));

readDeltaData(fullfile(root, datasetName));
...
outImage = uint8(zeros(imageWidth,imageHeight,imageDepth));
% for c=4:min([imageDatasets(:).NumberOfChannels])
c=4;
    fprintf('Chan:%d',c);
    for t=1:min([imageDatasets(:).NumberOfFrames])
