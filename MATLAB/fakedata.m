...
im1Lims = im1Lims - [60 50 20;60 50 20];
im2Lims = im2Lims + [10 10 10;10 10 10];
...
% figure;image(255*max(im1,[],3));colormap(gray);
% figure;image(255*max(im2,[],3));colormap(gray);
...
fprintf(fid, 'XPosition:%f\n', round(im1Lims(2)+delta1(1))/1e6);
fprintf(fid, 'YPosition:%f\n', round(im1Lims(1)+delta1(2))/1e6);
fclose(fid);
...
fprintf(fid, 'YPixelPhysicalSize:1\n');
fprintf(fid, 'ZPixelPhysicalSize:1\n');
fprintf(fid, 'XPosition:%f\n', round(im2Lims(2)+delta2(1))/1e6);
fprintf(fid, 'YPosition:%f\n', round(im2Lims(1)+delta2(2))/1e6);
fclose(fid);
