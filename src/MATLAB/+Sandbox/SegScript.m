...
montages(end).filePath = 'P:\Images\Temple\3d\SVZ\Montage\Contralateral Sides 7-2-15\DAPI EdU-Cy5 GFAP-488 Dcx-Cy3 22mSVZ w2mChP-CM\DAPI EdU-Cy5 GFAP-488 Dcx-Cy3 22mSVZ w2mChP-CM #_Montage_wDelta\';
montages(end).chanList = [1,2,4];
montages(end).faceColor = [0,1,0];
montages(end).marker = 'v';

montages(end+1).filePath = 'P:\Images\Temple\3d\SVZ\Montage\Contralateral Sides 7-2-15\DAPI EdU-Cy5 GFAP-488 Dcx-Cy3 22mSVZ w22mChP-CM\DAPI EdU-Cy5 GFAP-488 Dcx-Cy3 22mSVZ w22mChP-CM #_Montage_wDelta\';
montages(end).chanList = [1,2,4];
montages(end).faceColor = [1,0,0];
montages(end).marker = 's';

montages(end+1).filePath = 'P:\Images\Temple\3d\SVZ\Montage\Contralateral Sides 7-2-15\DAPI EdU-Cy5 GFAP-488 Dcx-Cy3 22mSVZ wControlPump1 10x1\DAPI EdU-Cy5 GFAP-488 Dcx-Cy3 22mSVZ wControlPump1 10x1 #_Montage_wDelta\';
montages(end).chanList = [1,2,4];
montages(end).faceColor = [0,0,1];
montages(end).marker = 'o';

montages(end+1).filePath = 'P:\Images\Temple\3d\SVZ\Montage\Contralateral Sides 7-2-15\DAPI EdU-Cy5 GFAP-488 Dcx-Cy3 22mSVZ wControlPump1 10x2\DAPI EdU-Cy5 GFAP-488 Dcx-Cy3 22mSVZ wControlPump1 10x2 #_Montage_wDelta\';
montages(end).chanList = [1,2,4];
montages(end).faceColor = [0,0,1];
montages(end).marker = 'o';

montages(end+1).filePath = 'P:\Images\Temple\3d\SVZ\Montage\Contralateral Sides 9-18-15\DAPI EdU-647 GFAP-488 Dcx-Cy3 22mSVZ StabSham1 10x1_4x01\DAPI EdU-647 GFAP-488 Dcx-Cy3 22mSVZ StabSham 10x1_4x01 #_Montage_wDelta\';
montages(end).chanList = [1,2,4];
montages(end).faceColor = [0,0,1];
montages(end).marker = 'o';

montages(end+1).filePath = 'P:\Images\Temple\3d\SVZ\Montage\Contralateral Sides 9-18-15\DAPI EdU-647 GFAP-488 Dcx-Cy3 22mSVZ w2mChP-CM 2 10x1_4x01\DAPI EdU-647 GFAP-488 Dcx-Cy3 22mSVZ w2mChP-CM 2 10x1_4x01 #_Montage_wDelta\';
montages(end).chanList = [1,2,4];
montages(end).faceColor = [0,1,0];
montages(end).marker = 'v';

montages(end+1).filePath = 'P:\Images\Temple\3d\SVZ\Montage\Contralateral Sides 9-18-15\DAPI EdU-647 GFAP-488 Dcx-Cy3 22mSVZ w22mChP-CM 1 10x1_4x01\DAPI EdU-647 GFAP-488 Dcx-Cy3 22mSVZ w22mChP-CM 1 10x1_4x01 #_Montage_wDelta\';
montages(end).chanList = [1,2,4];
montages(end).faceColor = [1,0,0];
montages(end).marker = 's';

montages(end+1).filePath = 'P:\Images\Temple\3d\SVZ\Montage\Contralateral Sides 9-18-15\DAPI EdU-647 GFAP-488 Dcx-Cy3 22mSVZ w22mChP-CM 2 10x1_4x01\DAPI EdU-647 GFAP-488 Dcx-Cy3 22mSVZ w22mChP-CM 2 10x1_4x01 #_Montage_wDelta\';
montages(end).chanList = [1,2,4];
montages(end).faceColor = [1,0,0];
montages(end).marker = 's';

ball = double(Cuda.MakeBallMask(2));
...
%     disp([fullfile(pathstr,['_',imData.DatasetName,'_RGB_mask.tif']),'\n']);
%     imMask = imread(fullfile(pathstr,['_',imData.DatasetName,'_RGB_mask.tif']));
...
%     imChans = MicroscopeData.Reader(montages(i).filePath,[],montages(i).chanList);
%     readTime = toc(readStart);
%     [numBits,minVal,maxVal] = Utils.GetClassBits(imChans);
%     
%     processStart = tic;
%     for c=1:length(montages(i).chanList)
%         im = imChans(:,:,:,c);
%         %             im = Cuda.CudaMex('ContrastEnhancement',im,[250,250,175],[3,3,3]);
%         im = Denoise.FluorescentBGRemoval(im);
%         %im = Cuda.CudaMex('MorphologicalClosure',im,ball);
%         
%         %im = Cuda.CudaMex('MedianFilter',im,[3,3,3]);
%         im(~imMask) = 0;
...
%         Utils.PrintTime(fullTime),Utils.PrintTime(readTime),Utils.PrintTime(processTime),Utils.PrintTime(writeTime));
%     %end
% end
...
for i=1:length(montages)
    ...
    im = MicroscopeData.Reader(montages(i).filePath,[],montages(i).chanList(3));
    imMask = imread(fullfile(pathstr,['_',imData.DatasetName,'_RGB_mask.tif']));
    imMask = repmat(imMask,1,1,imData.ZDimension);
    ...
    imDataB = MicroscopeData.ReadMetadata(fullfile(montages(i).filePath,[imData.DatasetName,'_seg']));
    MicroscopeData.Writer(imBW,pathstr,imDataB,[],3);
    fprintf('Done: %s\n',Utils.PrintTime(toc))
end
